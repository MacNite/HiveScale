...
> On the **XIAO ESP32-C6** (firmware 0.25.0+) the setup button is the board's on-board USER/BOOT button, and the external D2 button is the [inspection button](docs/inspection-mode.md). The 30-pin board above is unchanged. Note that GPIO9 cannot wake the C6 from deep sleep and holding it at power-up enters the serial bootloader, so on that board press and hold the USER button and the AP opens a second into the next wake (up to one send interval away), or use **Start AP mode** in the dashboard for no wait at all — see [docs/ap-mode-sd-download.md](docs/ap-mode-sd-download.md).
