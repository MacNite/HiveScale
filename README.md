...
- **Listen to a hive (optional)** — ask a paired HiveInside node (firmware 0.6.0+) for live audio and hear it in the dashboard, or play back an earlier session. Recording is authenticated with a shared key, capped at 60 seconds per session, and stored as WAV on your own server. Useful for checking what an insight flagged — or just for listening to the bees. See [Listening to a hive](docs/audio-recording.md).
