...
- **Hive audio (optional)** — ask a paired HiveInside node (firmware 0.6.2+) to record 10–60 seconds from inside the hive, then play it back in the dashboard. The hub collects the request on its next wake, so a recording arrives within one reporting interval rather than instantly. Authenticated with a shared key and stored as WAV on your own server. Useful for checking what an insight flagged — or just for listening to the bees. See [Listening to a hive](docs/audio-recording.md).
